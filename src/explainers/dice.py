...
class DicePyTorchExplainer(ExplainerBase):

    def __init__(self, data_interface, model_interface):
        """Init method

        :param data_interface: an interface class to access data_loaders related params.
        :param model_interface: an interface class to access trained ML model_loaders.

        """

        super().__init__(data_interface) # initiating data_loaders related parameters

        # initializing model_loaders variables
        self.model = model_interface

        # loading trained model_loaders
        self.model.load_model()

        # set the model_loaders in evaluation mode
    ...
    def generate_counterfactuals(self, query_instance, total_CFs, budget, desired_class="opposite", proximity_weight=0.5, diversity_weight=1.0, categorical_penalty=0.1, algorithm="DiverseCF", features_to_vary="all", yloss_type="hinge_loss", diversity_loss_type="dpp_style:inverse_dist", feature_weights="inverse_mad", optimizer="pytorch:adam", learning_rate=0.05, min_iter=500, max_iter=5000, project_iter=0, loss_diff_thres=1e-5, loss_converge_maxiter=1, verbose=False, init_near_query_instance=True, tie_random=False, stopping_threshold=0.5, posthoc_sparsity_param=0.1, posthoc_sparsity_algorithm="linear", seed=None):
        ...
        :param posthoc_sparsity_algorithm: Perform either linear or binary search. Takes "linear" or "binary". Prefer binary search when a feature range is large (for instance, income varying from 10k to 1000k) and only if the features share a monotonic relationship with predicted outcome in the model_loaders.

        :return: A CounterfactualExamples object to store and visualize the resulting counterfactual explanations (see counterfactuals_example.py).
        ...
        # PY:  For the features to vary, samples random cfs with their min-max ranges.
        ...
        # PY: Initialized BCE loss and feature weights for it.
        if([yloss_type, diversity_loss_type, ] != self.loss_weights):
            self.do_loss_initializations(yloss_type, diversity_loss_type, feature_weights)
            # PY: Set the weights for different loss terms appropriately.
        ...
        query_instance, test_pred = self.find_counterfactuals(query_instance, desired_class, optimizer,
        ...
        return cf_exp.CounterfactualExamples(self.data_interface, query_instance, test_pred, self.final_cfs,
    ...
    def do_cf_initializations(self, total_CFs, algorithm, features_to_vary):
        ...
        if features_to_vary != self.features_to_vary:
            ...
            self.feat_to_vary_idxs = self.data_interface.get_indexes_of_features_to_vary(features_to_vary)
    ...
    def do_loss_initializations(self, yloss_type, diversity_loss_type, feature_weights):
        ...
        if feature_weights != self.feature_weights_input:
            ...
            # PY: Features weights for continuous computed for others its 1.
    ...
    def compute_yloss(self):
        ...
        for i in range(self.total_CFs):
            prob_1 = self.predict_probs(self.cfs[i], return_numpy=False).unsqueeze(0)
            if self.yloss_type == "l2_loss":
                temp_loss = torch.pow((prob_1 - self.target_cf_class), 2)[0]
            elif self.yloss_type == "log_loss":
                temp_logits = torch.log((abs(prob_1 - 0.000001))/(1 - abs(prob_1 - 0.000001)))
    ...
    def compute_dist(self, x_hat, x1):
        ...
        # return torch.sum(torch.mul((x_hat - x1), self.feature_weights_list), dim=0)
    ...
    def round_off_cfs(self, assign=False):
        ...
        for index, tcf in enumerate(self.cfs):
            ...
            for i, v in enumerate(self.encoded_continuous_feature_indexes):
                # PY: inverse transform to get into original scale/ Un-normalisation.
                # PY: this step un-normalizes the cf values and then rounds it off to the desired precision
                # and then normalises it again.
            ...
            # PY: Discretizing within the categorical features. setting the max to 1 all others 0.
            # PY: Iterating over the cate features, v contains the indices of the ohe features of a particular features.
            for v in self.encoded_categorical_feature_indexes:
                maxs = np.argwhere(
    ...
    def stop_loop(self, itr, loss_diff):
        ...
        if self.model.num_forward_pass > self.local_budget:
            return True
        ...
        if loss_diff <= self.loss_diff_thres:
            ...
            if self.loss_converge_iter < self.loss_converge_maxiter:
                ...
                test_preds = self.predict_probs(np.array(temp_cfs), return_last=True, return_numpy=True)
    ...
    def find_counterfactuals(self, query_instance, desired_class, optimizer, learning_rate, min_iter, max_iter, project_iter, loss_diff_thres, loss_converge_maxiter, verbose, init_near_query_instance, tie_random, stopping_threshold, posthoc_sparsity_param, posthoc_sparsity_algorithm):
        ...
        PY: takes in a query (dict or list) and returns the query in the real space along with its predicts.
        Creates attributes containing the counterfactuals.
        """

        # Prepares user defined query_instance for DiCE.
        # PY: The out is a df with ohe features.
        ...
        test_pred = self.predict_probs(query_instance, return_last=True, return_numpy=True)
        ...
        if loop_find_CFs == 1:
            self.local_budget = self.budget - self.total_CFs
        else:
            self.local_budget = (self.budget-self.model.num_forward_pass)//loop_find_CFs
            raise NotImplementedError('Stopping criteria not implemented for this case.')

        for _ in range(loop_find_CFs):
            # PY: initialise counterfactual either near the query instance or randomly between its min and max.
            ...
            while self.stop_loop(iterations, loss_diff) is False:
                ...
                # projection step, clamp between min and max values.
                for ix in range(self.total_CFs):
                    for jx in range(len(self.minx[0])):
                ...
                test_preds_stored = self.predict_probs(np.array(temp_cfs_stored), return_last=True, return_numpy=True)
                ...
                if((self.target_cf_class == 0 and all(i <= self.stopping_threshold for i in test_preds_stored)) or (self.target_cf_class == 1 and all(i >= self.stopping_threshold for i in test_preds_stored))):
                    ...
                    if avg_preds_dist < self.min_dist_from_threshold: # PY: Should this be other way?
        ...
        self.cfs_preds = self.predict_probs(np.array(self.final_cfs), return_last=True, return_numpy=True)
        ...
        # post-hoc operation on continuous features to enhance sparsity - only for public data_loaders
        ...
        if self.valid_cfs_found:
            ...
            for pred in self.cfs_preds:
                if((self.target_cf_class == 0 and pred[0] < self.stopping_threshold) or (self.target_cf_class == 1 and pred[0] > self.stopping_threshold)):
                    self.total_CFs_found += 1
            print('Only %d (required %d) Diverse Counterfactuals found for the given configuation' % (self.total_CFs_found, self.total_CFs), '; total time taken: %02d' % m, 'min %02d' % s, 'sec')
